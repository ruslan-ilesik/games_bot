...
template <typename T> struct Task {
  ...
  using handle_type =
      std::coroutine_handle<promise_type>; ///< Type alias for the coroutine
                                           ///< handle
  ...
  struct Awaitable {
    ...
    bool await_ready() {
      return coro.done();
    } ///< Checks if the coroutine is already done
    void await_suspend(std::coroutine_handle<> h) {
      coro.resume();
    } ///< Resumes the coroutine when suspended
    T await_resume() {
  ...
  } ///< Provides co_await support for the task
  ...
  struct promise_type {
    ...
    std::function<void()>
        continuation; ///< Optional continuation to be called on completion
    ...
    void unhandled_exception() {
      std::terminate();
    } ///< Terminates if an exception is unhandled
  };
};
...
template <> struct Task<void> {
  ...
  struct promise_type {
    ...
    void return_void() {
      result = 0;
    } ///< Sets result to 0 to indicate completion for void tasks
    void unhandled_exception() {
      std::terminate();
    } ///< Terminates if an exception is unhandled
  };
