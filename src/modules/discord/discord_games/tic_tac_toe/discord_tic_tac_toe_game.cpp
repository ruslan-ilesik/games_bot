...
namespace gb {
void Discord_tic_tac_toe_game::create_image(dpp::message &m,
                                            dpp::embed &embed) {
  _img_cnt++;
  double size = 256;
  ...
  image_generator_t base = [](const Image_processing_ptr &image_processing,
    ...
    if (resolution.x != resolution.y) {
      throw std::runtime_error(
          "tic tac toe base image supports only squared image");
    }
    int size = resolution.x;
    for (int y = 0; y < 3; y++) {
      image->draw_line({0, (y * size / 3)}, {size, (y * size / 3)},
                       {255, 255, 255}, size / 47);
      image->draw_line({(y * size / 3), 0}, {(y * size / 3), size},
                       {255, 255, 255}, size / 47);
    }
...
dpp::task<void>
Discord_tic_tac_toe_game::run(const dpp::button_click_t &_event) {
  game_start();
  ...
  while (1) {
    ...
    embed.set_color(dpp::colors::blue)
        ...
        .set_description(std::format(
            ...
            std::chrono::duration_cast<std::chrono::seconds>(
                ...
                60,
            dpp::utility::user_mention(get_current_player()),
            static_cast<SIGNS>(get_current_player_index()) == SIGNS::O ? "0"
                                                                       : "X"));
    create_image(m, embed);
    create_components(m);
    m.add_embed(embed);
    auto button_click_awaiter =
        _data.button_click_handler->wait_for(m, {get_current_player()}, 60);
    ...
    if (!std::isdigit(event.custom_id[0]) ||
        !std::isdigit(event.custom_id[1])) {
      id = "00";
    }
