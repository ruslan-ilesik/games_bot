                    ...
                    if (players_amount < 1 || players_amount > 6) {
                        _bot->reply(event, dpp::message().add_embed(
                                               dpp::embed()
                                                   ...
                                                   .set_title("Error!")
                                                   .set_description(
                                                       "players amount should be between 1 and 6, while you gave " +
                                                       std::to_string(players_amount))));
                    } else {
                        PREMIUM_STATUS status = co_await _premium->get_users_premium_status(event.command.usr.id);
                        time_t last_played =
                            co_await _games_manager->get_seconds_since_last_game("hangman", event.command.usr.id);
                        if (status == PREMIUM_STATUS::NO_SUBSCRIPTION && (players_amount > 2 || players.size() > 2)) {
                            ...
                            if (players_amount == 1 && players.size() == 1) {
                                ...
                                Lobby_return r =
                                    co_await this->lobby(event, players, event.command.usr.id, players_amount);
                                if (!r.is_timeout) {
