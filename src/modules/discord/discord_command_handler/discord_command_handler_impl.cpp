...
namespace gb {
    ...
    void Discord_command_handler_impl::do_command_remove(const std::string &name) {
        if (!_commands.contains(name)) {
            throw std::runtime_error("Discord_command_handler error: command " + name +
    ...
    void Discord_command_handler_impl::run() { set_bulk(false); }
    ...
    void Discord_command_handler_impl::init(const Modules &modules) {
        ...
        this->_insert_command_use_stmt =
            _db->create_prepared_statement("INSERT INTO  `commands_use` (`command`, `time`, `user_id`, `channel_id`, "
                                           "`guild_id`) VALUES (?, UTC_TIMESTAMP(), ?, ?, ? )");
        ...
        _discord_bot->add_pre_requirement([this]() {
            this->_on_slashcommand_handler =
                _discord_bot->get_bot()->on_slashcommand([this](const dpp::slashcommand_t &event) -> dpp::task<void> {
                    std::shared_lock<std::shared_mutex> lock(_mutex);
                    if (!_commands.contains(event.command.get_command_name())) {
                        _discord_bot->get_bot()->log(dpp::ll_error,
                                                     "Command " + event.command.get_command_name() + " was not found");
                        co_return;
                    }
                    ...
                    get_full_command_name = [&get_full_command_name](auto cmd_data) -> std::string {
                        if (cmd_data.options.size()) {
                            ...
                            if (subcommand.type == dpp::co_sub_command_group ||
                                subcommand.type == dpp::co_sub_command) {
                                return " " + subcommand.name + get_full_command_name(subcommand);
                            }
                            return "";
                        }
                        return "";
                    ...
                    dpp::command_interaction data = interaction.get_command_interaction();
                    std::string name = event.command.get_command_name();
                    if (!data.options.empty() && (data.options[0].type == dpp::co_sub_command_group ||
                                                  data.options[0].type == dpp::co_sub_command)) {
        ...
        _admin_terminal->add_command("discord_command_handler_get_bulk", "Command to get current value of bulk status.",
                                     "Arguments: no arguments", [this](const std::vector<std::string> &arguments) {
                                         std::cout << "Command discord_command_handler_get_bulk: bulk: "
                                                   << (_bulk ? "true" : "false") << std::endl;
                                     });

        _admin_terminal->add_command("discord_command_handler_bulk_disable",
                                     "Command to change discord command handler bulk create option to false, will "
                                     "register all commands in waiting list.",
                                     "Arguments: no arguments",
                                     [this](const std::vector<std::string> &arguments) { set_bulk(false); });

        _admin_terminal->add_command(
            "discord_command_handler_bulk_enable",
            "Command to change discord command handler bulk create option to true, will start adding commands to "
            "buffer to register until run command is called or this option set to false.",
            "Arguments: no arguments.", [this](const std::vector<std::string> &arguments) { set_bulk(true); });
        ...
        _admin_terminal->add_command(
            "discord_command_handler_remove_command", "Command to remove a command by name.",
            "Arguments: command name.", [this](const std::vector<std::string> &arguments) {
                if (arguments.empty()) {
                    std::cout << "discord_command_handler_remove_command command error: no command name were provided"
                              << std::endl;
                    return;
                }
                try {
                    ...
                    std::cout << "discord_command_handler_remove_command command error: error removing command"
                              << std::endl;
                }
            });
    }
    ...
    void Discord_command_handler_impl::register_command(const Discord_command_ptr &command) {
        ...
        if (_commands.contains(command->get_name())) {
            throw std::runtime_error("Discord_command_handler error: Command " + command->get_name() +
                                     " already registered");
        }
        if (_bulk) {
            _command_register_queue.push_back(command->get_name());
        } else {
            _discord_bot->get_bot()->global_command_create(
                command->get_command(), [this](const dpp::confirmation_callback_t &event) {
                    if (event.is_error()) {
