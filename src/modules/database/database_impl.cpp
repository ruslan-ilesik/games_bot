...
namespace gb {
    ...
    void Database_impl::init(const Modules &modules) {
        ...
        _admin_terminal->add_command("database_add_connection", "Adds new database connection to pool",
                                     "Arguments: no arguments.", [this](const std::vector<std::string> &arguments) {
                                         try {
                                             new_connection();
                                         } catch (...) {
                                             std::cout
                                                 << "database_add_connection command error: Error adding connection"
                                                 << std::endl;
                                         }
                                     });

        _admin_terminal->add_command(
            "database_get_connections", "Prints all connection from pool and their status", "Arguments: no arguments.",
        ...
        _admin_terminal->add_command(
            "database_request_queue_size", "Shows length of request queue (including currently running)",
            "Arguments: no arguments.", [this](const std::vector<std::string> &arguments) {
                try {
                    std::unique_lock lock(_bg_thread_mutex);
                    std::cout << "Database queries in queue: " << (queries_amount - _bg_queue.size()) << std::endl;
                } catch (...) {
                    std::cout << "database_remove_connection command error: Error getting request queue size"
                              << std::endl;
                }
            });
    ...
    Task<Database_return_t> Database_impl::execute(const std::string &sql) {
        ...
        std::function<Task<Database_return_t>()> tt = [sql, evaluated, this, storage,
                                                       storage_mutex]() -> Task<Database_return_t> {
    ...
    Database_impl::_execute_prepared_statement(Prepared_statement st,
    ...
    Module_ptr create() { return std::dynamic_pointer_cast<Module>(std::make_shared<Database_impl>()); }
    ...
    void Mysql_connection::_prepare_statement(const Prepared_statement &st) {
        ...
        if (mysql_stmt_prepare(stm, statement.c_str(), statement.size()) != 0) {
            ...
            _log->critical("Database ERROR: Failed to create prepared statement: " + statement +
                           ". MySQL error: " + error_message);
            throw std::runtime_error("Database ERROR: Failed to create prepared statement: " + statement +
                                     ". MySQL error: " + error_message);
        }
    ...
    Database_return_t Mysql_connection::execute_prepared_statement(const Prepared_statement &st,
                                                                   std::unique_ptr<Prepared_statement_params> params) {
        int max_retries = 2;
        int attempts = 0;
        ...
        while (attempts < max_retries) {
            try {
                ...
                if (mysql_stmt_execute(stmt)) {
                    _log->critical("Database Error: mysql_stmt_execute(), failed. Error: " +
                                   std::string(mysql_stmt_error(stmt)));
                    throw std::runtime_error("Database Error: mysql_stmt_execute(), failed. Error: " +
                                             std::string(mysql_stmt_error(stmt)));
                }
                ...
                do {
                    ...
                    if (result) {
                        _log->critical("Database ERROR: mysql_stmt_bind_result failed: " +
                                       std::string(mysql_stmt_error(stmt)));
                        throw std::runtime_error("Database ERROR: mysql_stmt_bind_result failed: " +
                                                 std::string(mysql_stmt_error(stmt)));
                    }
                    ...
                    while (true) {
                        ...
                        if (mysql_num_fields(confres)) {
                            ...
                            while (s_field_count < mysql_num_fields(confres)) {
                                ...
                                std::string b =
                                    is_null[s_field_count]
                                        ? ""
                                        : std::string(string_buffers[s_field_count].get(), lengths[s_field_count]);
                                row[a] = b;
                                s_field_count++;
                            }
    ...
    std::vector<MYSQL_BIND> Mysql_prepared_statement_params::get_binds() { return _binds; }
} // namespace gb
